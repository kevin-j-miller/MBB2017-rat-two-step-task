function normLik = xval_twostep_stan(data_rat,inc)
    ...
    %fit_even = stan('file','multiagent_model_single_xval_HMM.stan','data',standata_even,'verbose',true,'chains',5,'warmup',500,'iter',200,'working_dir',wd);
    
    fit_even = stan('file','multiagent_model_single_xval_HMM.stan','data',standata_even,'verbose',false,'method','optimize','working_dir',wd);
    fit_even.block();
    fit_even_extracted = extract(fit_even);
    
    fit_odd = stan('file','multiagent_model_single_xval_HMM.stan','data',standata_odd,'verbose',false,'method','optimize','working_dir',wd);
    fit_odd.block();
